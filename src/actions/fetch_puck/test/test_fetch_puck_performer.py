...
class TestFetch_puckPerformer(unittest.TestCase):
	def setUp(self):
		# Init some position
		...
		# Perform client
		self.pc = PerformClient("test_fetch_puck_performer_client", "actions")
	...
	def test_action_point(self):
		"""
			Test action point computation for fetch_puck performer.
		"""
		# Create move action
		action = Action.parse_string("<fetch_puck />")
	...
	def test_perform_success(self):
		self.action_state = ActionStatus.IDLE
		self.pc.perform_action(Action.parse_string("<fetch_puck />"), Pose2D())
...
if __name__ == '__main__':
	rospy.init_node('test_fetch_puck_performer', anonymous=True)
	rostest.rosrun('fetch_puck', 'actions', TestFetch_puckPerformer, sys.argv)
